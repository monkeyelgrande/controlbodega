 ...
 */
public class frm_cotizacion extends javax.swing.JInternalFrame {
    ...
    public frm_cotizacion() throws IOException {
        initComponents();

        TextPrompt orden = new TextPrompt("No. Cotización", txt_codigo);
        ...
        lbl_numerofactura.setText(DB_consultas_R_D.cargarId("cotizaciones_cabeceras"));
...
//        System.out.println(consulta);
    ...
    private void txt_codigoFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_txt_codigoFocusLost
        if (!txt_codigo.getText().equals("")) {
            if (DB_consultas_R_D.consultar_existencia_campo_String("codigo", txt_codigo.getText(), "cotizaciones_cabeceras") >= 1) {
                JOptionPane.showMessageDialog(this, "EL codigo ingresado ya esta asosciado a otra cotización");
                txt_codigo.setText("");
    ...
    private void btn_facturarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_facturarActionPerformed
        if (jtabla_Ventas.getRowCount() < 1) {
            ...
            for (int i = 0; i < jtabla_Ventas.getRowCount(); i++) {
                ...
                double stock = DB_consultas_R_D.consultar_stock("" + modelo_ventas.getValueAt(i, 1));
                if (can > stock) {
                    flag = false;
                    int dialogButton = JOptionPane.YES_NO_OPTION;
            ...
            if (flag) {
                ...
                if (dbfactura.Guardar_Cotizacion(fc) == 1) {
                    productos = new ArrayList<ProductoImprimir>();
                    ...
                    for (int i = 0; i < jtabla_Ventas.getRowCount(); i++) {
                        ...
                        SSQL += "INSERT INTO cotizaciones_detalles (id,id_cabecera,id_producto,cantidad, subtotal, id_factura) "
                    ...
                    try {
                        ...
                        Logger.getLogger(frm_cotizacion.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
                ...
                if (imprimirSiNo == 1) {
                    int dialogButton = JOptionPane.YES_NO_OPTION;
                    int dialogResult = JOptionPane.showConfirmDialog(null, "¿Desea imprimir la cotización?", "Alerta", dialogButton);
                    if (dialogResult == JOptionPane.YES_OPTION) {
...
//            System.out.println(consulta);
    ...
    private void btn_buscar_clienteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_buscar_clienteActionPerformed
        ...
        buscar_cleinte.formulario="cotizacion";
        buscar_cleinte.show();
