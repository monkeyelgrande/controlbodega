...
import Formularios_internos.jd_cotizacion;
...
/**
 * Listado de solicitudes de cotización de compra (RFQ, RF-03).
 *
 * @author Monkeyelgrande
 */
public class frm_cotizaciones_compra extends javax.swing.JInternalFrame {
    ...
    public frm_cotizaciones_compra() {
        initUI();
        actualizar();
    ...
    private void initUI() {
        ...
        setMaximizable(true);
        setResizable(true);
        setTitle("Cotizaciones de compra (RFQ)");
        setSize(980, 560);

        JPanel root = new JPanel(new BorderLayout());
        root.setBackground(EstiloCompras.BG_FORM);
        root.add(EstiloCompras.header(FontAwesome.FILE_INVOICE, "Solicitudes de cotización (RFQ)", null), BorderLayout.NORTH);
        ...
        jbox_estado = new JComboBox<>(new String[]{"Todos", "Borrador", "Enviada", "Respondida", "Sin respuesta"});
        ...
        modelo.setColumnIdentifiers(new Object[]{"id", "N°", "Proveedor", "Fecha", "Estado", "Items"});
        jtabla = new JTable(modelo);
        EstiloCompras.styleTable(jtabla);
        EstiloCompras.anchoColumnas(jtabla, 50, 90, 260, 110, 130, 70);
    ...
    public final void actualizar() {
        ...
        ResultSet rs = DBcotizaciones.listar(estadoFiltro());
        try {
            while (rs.next()) {
                modelo.addRow(new Object[]{
                    rs.getString("id"), rs.getString("numero"), rs.getString("proveedor"),
                    rs.getDate("fecha"), DBcotizaciones.nombreEstado(rs.getInt("estado")),
                    rs.getString("items")});
            }
            rs.close();
        } catch (Exception e) {
    ...
    private void abrir() {
        ...
        if (f < 0) {
            JOptionPane.showMessageDialog(this, "Seleccione una cotización");
            return;
        }
        int id = Integer.parseInt(jtabla.getValueAt(f, 0).toString());
        new jd_cotizacion(id).setVisible(true);
        actualizar();
    }
}
