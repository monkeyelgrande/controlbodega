 ...
 */
public class frm_fondos extends javax.swing.JInternalFrame {
    ...
    public int consultarId(String id) {
        ResultSet rs = DB_consultas_R_D.getTabla("select count(id) as id from fondos where id = " + id + "");
        try {
            while (rs.next()) {
    ...
    private void btn_eliminarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_eliminarActionPerformed
        ...
        if (fila == -1) {
            ...
            int dialogResult = JOptionPane.showConfirmDialog(null, "Desea eliminar este fondo?", "Alerta", dialogButton);
            if (dialogResult == JOptionPane.YES_OPTION) {
                ...
                try {
                    ...
                    DB_consultas_R_D.eliminar("fondos", id);
        ...
        if (fila < 0) {
            ...
            try {
                while (rs.next()) {
                    ...
                    jif_crear_fondo.txt_nombre.setText(rs.getString("nombre"));

                    jif_crear_fondo.chk_predeterminado.setSelected(false);
                    if (rs.getInt("predeterminado") == 1) {
                        jif_crear_fondo.chk_predeterminado.setSelected(true);
                    }
                    // Convencion heredada de cajadiaria: 0 = fisico, 1 = digital
                    if (rs.getInt("fisico_digital") == 0) {
                        jif_crear_fondo.rbtn_fisico.setSelected(true);
                    } else {
                        jif_crear_fondo.rbtn_digital.setSelected(true);
                    }
                }
                rs.close();

            } catch (SQLException ex) {
                Logger.getLogger(frm_fondos.class.getName()).log(Level.SEVERE, null, ex);
            }
            if (ver) {
    ...
    public void actualizar() {
        ...
        // Saldo por fondo = SUM(ingresos.total) - SUM(egresos.total) del fondo (esquema sin abonos)
        ...
        try {
            while (rs.next()) {
                // añade los resultado a al modelo de tabla
                String pre = "Predetermiando";
                if (rs.getInt("predeterminado") != 1) {
                    pre = "";
                }
                String saldo = formateador.format(rs.getDouble("saldo"));
                modelo.addRow(new Object[]{rs.getString("id"), rs.getString("nombre"), saldo, pre});
            }
            rs.close();
            // asigna el modelo a la tabla
            jtabla_clientes.setModel(modelo);
